...
const idleFrames = importAll(import.meta.glob("@/assets/sprites/Shadow_Fiend/Idle/*"));
...
const playNext = (name, infinite) => {
  ...
  switch (name) {
    ...
    case "damage":
      _damageAnimation.value?.play(1000, infinite);
      return _damageAnimation.value;
    case "death":
      endAnimationIndicator.value = true;
      _deathAnimation.value?.play(2567, infinite);
...
watchEffect(() => {
  if (props.userId && props.userLevel === 21) {
    deathFrames.value = importAll(import.meta.glob("/src/assets/sprites/Shadow_Fiend/Death/*"));
    damageFrames.value = importAll(import.meta.glob("/src/assets/sprites/Shadow_Fiend/Damage/*"));
  }
})

watchEffect(() => {
  if (deathFrames.value && damageFrames.value && canvas.value) {
    _deathAnimation.value = new TileAnimation(canvas.value, 1300, 1000, deathFrames.value);
    _damageAnimation.value = new TileAnimation(canvas.value, 1300, 1000, damageFrames.value);
...
onMounted(() => {
  _idleAnimation.value = new TileAnimation(canvas.value, 1300, 1000, idleFrames);
